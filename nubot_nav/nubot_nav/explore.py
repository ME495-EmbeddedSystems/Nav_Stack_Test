...
class Explore(Node):
    ...
    def map_callback(self, msg: OccupancyGrid):
        """Get map data and converts it to a 2D matrix."""
    ...
    def plotting(self):
        """Use to plot the graphs for visualisation."""
        plt.clf()
        plt.ion
        colors = {
            0: "white",  # Color for 0
        ...
        plt.plot(
            self.basex / self.resolution,
            self.basey / self.resolution,
            marker="o",
            c="green",
        )
    ...
    def use_index(self, y, x):
        """
        Convert positions to the index for the 2D map data matrix.
        ...
        print(x, y)
        if (
            abs(int(y / self.resolution)) < self.height - 1
            and abs(int(x / self.resolution)) < self.width - 1
        ):
            return self.maze_mat[
                abs(int(x / self.resolution)), abs(int(y / self.resolution))
            ]

        elif abs(int(y / self.resolution)) >= self.width - 1:
            return self.maze_mat[abs(self.width) - 1,
                                 abs(int(y / self.resolution))]

        elif abs(int(x / self.resolution)) >= self.width - 1:
            return self.maze_mat[abs(int(x / self.resolution)),
                                 abs(self.width) - 1]

    def timer_callback(self):
        """Control loop that runs and control states."""
        if self.data is not None:
            ...
            if self.nnum_zero == 0:
                if (
                    self.use_index(self.basex, self.basey - 3) == -1
                    or self.use_index(self.basex, self.basey - 3) == 0
                    and self.state == State.BOTTOM
                ):
                    self.goalx = self.basex + self.pos.x
                    self.goaly = self.basey - 1.0 + self.pos.y
                    self.goal()
                elif (
                    self.use_index(self.basex, self.basey - 3) > 95
                    and self.state == State.BOTTOM
                ):
                    self.state = State.RIGHT
                if (
                    self.use_index(self.basex - 3, self.basey) == -1
                    or self.use_index(self.basex - 3, self.basey) == 0
                ) and self.state == State.RIGHT:
                    self.goalx = self.basex - 1.0 + self.pos.x
                    self.goaly = self.basey + self.pos.y
                    self.goal()
                elif (
                    self.use_index(self.basex - 3, self.basey) > 95
                    and self.state == State.RIGHT
                ):
                ...
                if (
                    ...
                    self.state = State.RIGHT
                if (
                    self.use_index(self.basex + 2, self.basey) == -1
                    or self.use_index(self.basex + 2, self.basey) == 0
                    and self.state == State.LEFT
                ):
